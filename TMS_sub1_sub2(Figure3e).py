...
# Colors
professional_colors = {'Frontal-led': '#75BAFD', 'Hippocampus-led': '#FC6FB3'}
...
ax.errorbar(clinical_time_points, subtype0_means, yerr=subtype0_sems, fmt='-o', 
            color=professional_colors['Frontal-led'], label='Frontal-led', 
...
ax.errorbar(clinical_time_points, subtype1_means, yerr=subtype1_sems, fmt='-o', 
            color=professional_colors['Hippocampus-led'], label='Hippocampus-led', 
            capsize=3, capthick=1, elinewidth=2, markeredgewidth=2)

# Title and labels with adjusted title position
ax.set_title('HAM-D Score of Two MDD Subtypes Across the TMS Treatment Trial', fontsize=25, pad=20)
ax.set_xlabel('Time Points', fontsize=25)
ax.set_ylabel('HAM-D Score', fontsize=25)
ax.set_xticks(range(len(clinical_time_points)))
ax.set_xticklabels(clinical_time_points, rotation=45, fontsize=20)

# Adjusting y-axis ticks
ax.set_yticks(range(0, 26, 5))
ax.set_yticklabels(range(0, 26, 5), fontsize=16)
...
pdf_file_path = output_folder + 'TMS_plot_adjusted.pdf'
png_file_path = output_folder + 'TMS_plot_adjusted.png'
